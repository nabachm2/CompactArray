...
@interface DCompactArray_Tests : XCTestCase

@end

@implementation DCompactArray_Tests
...
-(void)testBasicSetGet {
    for (int bitNum = 2;bitNum < 62;bitNum ++) {
        ...
        long size = 100000;
        DCompactArray* array = new DCompactArray(size, bitNum, false);
        for (long i = 0;i < size;i ++)
            array->Set((int) i, i % maxValue);
        
        for (long i = 0;i < size;i ++)
            XCTAssertEqual(i % maxValue, array->Get((int) i));
...
-(void)testRandomSetGet {
    for (int bitNum = 2;bitNum < 62;bitNum ++) {
        ...
        srand(seedVal);
        for (long i = 0;i < size;i ++)
            array->Set((int) i, rand() % maxValue);
        
        srand(seedVal);
        for (long i = 0;i < size;i ++)
            XCTAssertTrue(rand() % maxValue == array->Get((int) i));
...
void testDifferentBitSizeHelperD(int bitNum) {
    ...
    srand((int)seedVal);
    for (long i = 0;i < size;i ++)
        assert((rand() % maxValue) == array->Get((int) i));
}
